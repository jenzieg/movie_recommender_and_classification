Make sure this format is what the capstone says
Figure out where all the saved data frames go to
...
### 1. Overview and Problem Statement <span id = "s1"></span>

#### Scenario
Have you ever found yourself searching through your tv network and streaming platforms looking for something to watch, only to realize you’ve wasted half of your time looking for a movie/show? If you are part of the 78% of consumers in the U.S. who use a subscription video-on-demand service [[1]](https://www.statista.com/statistics/318778/subscription-based-video-streaming-services-usage-usa/#:~:text=According%20to%20the%20most%20recent,25%20percent%20in%20five%20years.), the answer is probably yes.

While sometimes the algorithms recommending something to watch can be helpful, sometimes they are not. Many times, these platform services recommend a show but do not provide the ratings of others’ reviews to back up those recommendations.

A new streaming service’s production company has recognized this issue, and they are looking for a way to recommend movies to viewers differently than some of their competitors.
They are also looking for a new way to predict if their new movie releases will be rated as Fresh or Rotten by audience viewers on Rotten Tomatoes. Since Rotten Tomato critic reviews usually get published first, they want to predict what the audience will vote once the movie is released to the general public.
...
Pre-Processing – Recommender System
Recommender System Modeling and Evaluation
Website Development
Conclusions and Recommendations

### Data Collection and Cleaning <span id = "s2"></span>
...
Rotten tomato critic ratings range from 0-100%, and audience ratings range from 1-5 stars(or is it 0-5???). Any movie rating with an average of 59% or less is considered “Rotten,” while anything 60% or  higher is considered “Fresh.”

The IMDb dataset was also from 2020 and consisted of over 85K movie titles and user reviews. This dataset also had general movie information such as title, plot, director, etc., as well as the weighted movie ratings of users.

IMDb ratings range from 0-10, with zero being the lowest score and ten being the highest.

When combining the data, I merged the title (((and year??))) to create a main DataFrame that consisted of the following features:
...
After combining and cleaning the data, the primary dataset had 6587 movie titles and critic/audience reviews.
...
I ran these models with GridSearch using both the original features and polynomial features. The majority of the models performed better once the polynomial features were used. However, XYZ model performed worse or did not change.
...
| Model        | Gradient Boost | Bagging Classifier | Ada Boost |
|--------------|---------------------|---------------------  |
| Train Accuracy | 1.00 | 1.00 | 0.965  |
| Test Accuracy | 0.929 | 0.917 | 0.885 |
| Precision | 0.939 | 0.933 | 0.918 |
| Recall | 0.930 | 0.913 | 0.851 |
| Specificity | 0.927 | 0.921 | 0.944 |
| Misclassification | 0.708 | 0.082 | 0.155 |
| F1 Score | 0.935 | 0.923 | 0.883 |

All of these models used feature-engineered polynomial features. Although a bit overfit, the Gradient Boost model was best performing with a training accuracy score of 1.0 and test accuracy score of 0.929, which is X higher than the baseline model. This model used a 0.25 learning rate, a max depth of 5, and 300 estimators.

![gb](./images/gb_poly.png)

●   Misclassification Rate Interpretation: What percentage of observations did I incorrectly predict?
●   Specificity Interpretation: Among those I predicted were Rotten, how many did I get right
●   Recall: Among those who are Fresh, how many did I get right
●   Precision Interpretation: Among those I predicted were Fresh, how many did I get right

When evaluating the feature importance of this model, the most important predictor was the product of the score and the imdb_score features. This combined feature represented over 50% of the model’s ability to predict the target, audience_status. The second and third most important predictors were the product of the tomatometer_rating and score and the individual score column.
...
Overall, I created seven different recommender models, testing both Collaborative Filtering and Content Based Filtering.
...
I used transformers for my Content Based Filtering Models, including TF-IDF, Count Vectorizer, Word2Vec, and spaCy.
...
Content Based Filtering models with Word2Vec and spaCy transformations can be found in 05.2_Recommender_System.ipynb **(add this)**
...
**Recommender System** <br>
I would first recommend testing out other models that use NLP transformers and seeing if better results can be generated. The current model needs some fine-tuning, so I would recommend adding additional parameters and thoroughly testing results. It would be helpful to get user feedback as well, so maybe testing the model with a focus group or using A/B testing could help with improvements.

I think the model could benefit from additional ratings and reviews from other data sources, such as getting audience individual scores and ratings. I would also recommend getting tv show data and adding this as an option since there is a pivot from movies to tv shows???
...
<br>
[KNN Recommender System](https://towardsdatascience.com/how-did-we-build-book-recommender-systems-in-an-hour-part-2-k-nearest-neighbors-and-matrix-c04b3c2ef55c)<br>
[Word2Vec](https://www.kdnuggets.com/2020/08/content-based-recommendation-system-word-embeddings.html)
